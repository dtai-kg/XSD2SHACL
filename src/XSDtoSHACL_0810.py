...
class XSDtoSHACL:
    ...
    def parseXSD(self, XSD_FILE):
        """
        
        """
        if XSD_FILE not in self.XSD_Schemas:
            xsdTree = ET.parse(self.BASE_PATH + XSD_FILE)
            self.XSD_Schemas[XSD_FILE] = xsdTree.getroot()

    def queryElement(self, ref, root):
        """
        
        """
        self.root.find(f".//*[@name='{xsd_type}']",self.xsdNSdict)
    ...
    def transEleComplex(self,xsd_element):
        ...
        if element_type == None:
            ...
            next_node = self.root.find(f'.//xs:complexType[@name="{element_type}"]',self.xsdNSdict)
            # self.shapes.append(subject)
            return next_node #redirect current process to the next root (simple type)
    ...
    def translate(self,current_node):
        ...
        for child in current_node.findall("*"):
            ...
            if (("element" in tag) and (child.get("name") and element_type == 0)) or ("attribute" in tag)  or (("group" in tag) and (child.get("name") or child.get("id"))):
                self.shapes.pop()
        if self.backUp != None:
            self.shapes.pop()
            temp = self.backUp
    ...
    def evaluate_file(self, xsd_file):
        ...
        if "/" in xsd_file:
            self.BASE_PATH = xsd_file.split("/")[:-1]
        ...
        self.XSD_Schemas[xsd_file] = self.root
